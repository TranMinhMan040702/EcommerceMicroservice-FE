...
function Order() {
    ...
    const handleControl = (status) => {
        switch (status) {
            ...
            case 'SHIPPED':
                return <Link className="btn btn-outline-info rounded-0">Liên hệ người bán</Link>;
            case 'DELIVERED':
                return <Link className="btn btn-danger rounded-0 px-5">Đánh giá</Link>;
            case 'CANCELLED':
                return <Link className="btn btn-danger rounded-0">Mua lại</Link>;
            default:
                return (
                    <Link
                        className="btn btn-danger rounded-0"
