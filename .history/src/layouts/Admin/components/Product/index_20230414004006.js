...
function Product() {
    ...
    useEffect(() => {
        ProductService.getProducts(convertParams(params))
            .then((resp) => {
    ...
    const getAllProducts = async (convertParams(params)) {
        try {
            const response = await ProductService.getProducts(convertParams(params))
        } catch (err) {
            console.log(err)
        }
    }
