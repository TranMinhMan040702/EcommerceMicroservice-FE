...
function AddProduct() {
        ...
        if (id) {
            ProductService.getProductById(id)
                .then((resp) => {
                    ...
                    setImages(
                        resp.data.images.map((image, index) => {
                            const bits = image.split('.');
                            const file = new File([bits[0]], image, {
                                type: 'text/plain',
                            });
                            return {
                                name: `image${index + 1}`,
    ...
    const PublicProduct = async (e) => {
        ...
        if (!id) {
            ...
            await ProductService.updateProduct(data, product.id)
                .then((resp) => {
    ...
    return (
        <div className="product">
            <div className="content-header d-flex justify-content-between align-items-center">
                ...
                <Link to={config.routes.admin.product} className="btn btn-danger">
                    Go to products
                </Link>
            </div>
                                                        ...
                                                        <img
                                                            ...
                                                            src={
                                                                images.find((image) => image.name === `image${item}`)
                                                                    ...
                                                                    : PRODUCT_URL + '\\' + images[item - 1].file.name
                                                            }
                                                            alt=""
                                                            style={{
                                                                height: '200px',
